    ...
    def predict(
        ...
        Arguments:
          point_coords (np.ndarray or None): A Nx2 array of point prompts to the
            efficientvitsam. Each point is in (X,Y) in pixels.
          point_labels (np.ndarray or None): A length N array of labels for the
            point prompts. 1 indicates a foreground point and 0 indicates a
            background point.
          box (np.ndarray or None): A length 4 array given a box prompt to the
            efficientvitsam, in XYXY format.
          mask_input (np.ndarray): A low resolution mask input to the efficientvitsam, typically
            coming from a previous prediction iteration. Has form 1xHxW, where
            for SAM, H=W=256.
          multimask_output (bool): If true, the efficientvitsam will return three masks.
            For ambiguous input prompts (such as a single click), this will often
            produce better masks than a single prediction. If only a single
            mask is needed, the efficientvitsam's predicted quality score can be used
        ...
        Returns:
          ...
          (np.ndarray): An array of length C containing the efficientvitsam's
    ...
    def predict_torch(
        ...
        Returns:
          (torch.Tensor): The output masks in BxCxHxW format, where C is the
            number of masks, and (H, W) is the original image size.
          (torch.Tensor): An array of shape BxC containing the efficientvitsam's
            predictions for the quality of each mask.
...
def build_efficientvit_sam(image_encoder: EfficientViTSamImageEncoder, image_size: int, num_cls: int) -> EfficientViTSam:
...
def efficientvit_sam_l0(image_size: int = 512, num_cls=1, **kwargs) -> EfficientViTSam:
    ...
    return build_efficientvit_sam(image_encoder, image_size, num_cls)
