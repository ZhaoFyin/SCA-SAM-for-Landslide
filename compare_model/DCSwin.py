...
def windows_partition(x: torch.Tensor, window_size: int) -> Tuple[torch.Tensor, Tuple[int, int]]:
    """
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix, i):
        """ Forward function.
...
def dcswin_base(pretrained=True, num_classes=4, weight_path='./cp/stseg_base.pth'):
...
def dcswin_small(pretrained=True, num_classes=4, weight_path='./cp/stseg_small.pth'):
...
def dcswin_tiny(pretrained=True, num_classes=4, weight_path='./cp/stseg_tiny.pth'):
    model = DCSwin(encoder_channels=(96, 192, 384, 768),
                   num_classes=num_classes,
                   embed_dim=96,
                   depths=(2, 2, 6, 2),
