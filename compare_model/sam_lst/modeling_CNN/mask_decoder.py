...
class MaskDecoder(nn.Module):
    ...
    def predict_masks(
        ...
        b, c, h, w = src.shape
        if dense_prompt_embeddings.shape[-1] != src.shape[-1]:
            dense_prompt_embeddings = F.interpolate(dense_prompt_embeddings, size=(h, w), mode="bilinear",
                                                    align_corners=False)
        src = src + dense_prompt_embeddings

        pos_src = torch.repeat_interleave(image_pe, tokens.shape[0], dim=0)
        if pos_src.shape[-1] != src.shape[-1]:
