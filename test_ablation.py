...
def visualize_prediction(img_tensor, pred_tensor, gt_tensor, output_dir, file_name):
    """
    img_tensor: (3, H, W)
...
def save_result(rgb, pred, gt, result_dir, name, mode):
    if mode == "mask":
        if 255 in gt:
            mask_255 = (gt == 255).astype(np.uint8)
            pred[mask_255 == 1] = 255
        ...
        visualize_prediction(rgb, pred, gt, result_dir, name)
    else:
        return


def return_model(model_name, num_classes, lora_cfg):
    if "sca_enable" not in lora_cfg:
        lora_cfg["sca_enable"] = False
    assert model_name in ["SCA_SAM", "Full_Train", "Full_Frozen", "WO_SCA"]
    model = ScaSAM(lora_cfg,
                   num_classes=num_classes,
                   sca=True if model_name != "WO_SCA" else False,
                   img_s=512)
    return model
...
def main(dataset_name, model_name, save_mode):
    ...
    trained_dir = f"./results/{dataset_name}/{model_name}"
    train_cfg = os.path.join(trained_dir, "config.txt")
    train_cfg = Path(train_cfg).read_text(encoding="utf-8", errors="ignore")
    m = re.search(r"^\s*lora_cfg\s*:\s*(\{.*?\})\s*$", train_cfg, flags=re.MULTILINE | re.DOTALL)
    dict_str = m.group(1)
    lora_cfg = ast.literal_eval(dict_str)
    if dataset_name in ['YYL', 'BJL']:
        from dataset.landslide_dataset import LandslideDataset
        val_dataset = LandslideDataset(voc_root=data_dict[dataset_name], txt_name="test.txt")
    else:
        raise ValueError("Dataset not supported")

    val_loader = DataLoader(dataset=val_dataset, batch_size=1, shuffle=False)

    model = return_model(model_name, num_classes, lora_cfg=lora_cfg).cuda()
    weight_list = glob.glob(os.path.join(trained_dir, "*.pth"))
    best = max(weight_list, key=get_epoch)
    model.load_state_dict(torch.load(best), strict=True)

    save_dir = f"./vis_results_abl/{dataset_name}/{model_name}"
        ...
        for sampled_batch in t:
            ...
            for i in range(label_batch.shape[0]):
                ...
                save_result(rgb=image_batch[i].cpu().numpy(), pred=p, gt=l,
                            result_dir=save_dir, name=sampled_batch['img_id'][i], mode=save_mode)
...
if __name__ == '__main__':
    save_mode = "mask"
    for dataset_name in ["BJL", "YYL"]:
        for model_name in ["Full_Frozen", "SCA_SAM", "Full_Train",  "WO_SCA"]:
            main(dataset_name, model_name, save_mode)

"""
D:\Anaconda\envs\mss\python.exe G:\My_SAM_forL\test_ablation.py 
Full_Frozen on BJL dataset
mIoU	F1	OA	no_landslide	landslide
87.94	93.34	97.19	96.85	79.02
SCA_SAM on BJL dataset
mIoU	F1	OA	no_landslide	landslide
88.60	93.74	97.38	97.07	80.13
Full_Train on BJL dataset
mIoU	F1	OA	no_landslide	landslide
90.18	94.68	97.74	97.46	82.90
WO_SCA on BJL dataset
mIoU	F1	OA	no_landslide	landslide
87.35	92.98	97.10	96.77	77.93
Full_Frozen on YYL dataset
mIoU	F1	OA	no_landslide	landslide
81.86	89.30	96.86	96.65	67.08
SCA_SAM on YYL dataset
mIoU	F1	OA	no_landslide	landslide
87.35	92.92	97.90	97.74	76.96
Full_Train on YYL dataset
mIoU	F1	OA	no_landslide	landslide
88.48	93.62	98.11	97.97	78.99
WO_SCA on YYL dataset
mIoU	F1	OA	no_landslide	landslide
85.74	91.90	97.59	97.41	74.07
